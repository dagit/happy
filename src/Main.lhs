...
> import Data.List( nub, isSuffixOf, unionBy )
...
>       let mergeFlags flags cs = case getOpt Permute argInfo flags of
>             (fs,_,[])     -> return (unionBy cmpFlags cs fs)
>             (_ ,_,errors) -> errorOut errors
>       cli <- mergeFlags (getFlags ds) cli'
...
>         _ -> do
>          when (sr /= 0) $
>              hPutStrLn stderr ("shift/reduce conflicts:  " ++ show sr)
>          when (rr /= 0) $
>              hPutStrLn stderr ("reduce/reduce conflicts: " ++ show rr)
...
The point here is that we are comparing just the constructors, and
not the parameters to the constructors.

> cmpFlags :: CLIFlags -> CLIFlags -> Bool
#ifdef DEBUG
> cmpFlags DumpMangle         DumpMangle         = True
> cmpFlags DumpLR0            DumpLR0            = True
> cmpFlags DumpAction         DumpAction         = True
> cmpFlags DumpGoto           DumpGoto           = True
> cmpFlags DumpLA             DumpLA             = True
#endif
> cmpFlags DumpVersion        DumpVersion        = True
> cmpFlags DumpHelp           DumpHelp           = True
> cmpFlags (OptInfoFile  {})  (OptInfoFile  {})  = True
> cmpFlags (OptTemplate  {})  (OptTemplate  {})  = True
> cmpFlags (OptMagicName {})  (OptMagicName {})  = True
> cmpFlags OptGhcTarget       OptGhcTarget       = True
> cmpFlags OptArrayTarget     OptArrayTarget     = True
> cmpFlags OptUseCoercions    OptUseCoercions    = True
> cmpFlags OptDebugParser     OptDebugParser     = True
> cmpFlags OptStrict          OptStrict          = True
> cmpFlags (OptOutputFile {}) (OptOutputFile {}) = True
> cmpFlags OptGLR             OptGLR             = True
> cmpFlags OptGLR_Decode      OptGLR_Decode      = True
> cmpFlags OptGLR_Filter      OptGLR_Filter      = True
> cmpFlags _                  _                  = False
